...
const DICT = {
  nl: {
    ...
    // --- US-151: Plus -------------------------------------------------------
    Plus: 'Plus',
    'Upgrade to Plus': 'Upgrade naar Plus',
    Licence: 'Licentie',
    'Licence key': 'Licentiesleutel',
    'Paste the key from your confirmation mail. It is checked here, in the extension, against a public key: nothing is sent anywhere, and the key is stored outside your portfolio data. It is never part of an export or a bug report.':
      'Plak de sleutel uit je bevestigingsmail. Hij wordt hier, in de extensie, gecontroleerd tegen een publieke sleutel: er wordt niets verstuurd, en de sleutel staat los van je portefeuillegegevens. Hij zit nooit in een export of een bugrapport.',
    'Check and save': 'Controleren en bewaren',
    Remove: 'Verwijderen',
    'No licence entered. Everything on this page except the Plus screens works without one.': 'Geen licentie ingevoerd. Alles op deze pagina werkt zonder, behalve de Plus-schermen.',
    'This build carries no licence key yet, so no token can be checked. Plus is in preparation.': 'Deze versie draagt nog geen licentiesleutel, dus er kan nog geen sleutel gecontroleerd worden. Plus is in voorbereiding.',
    'That is not an Asteria licence. A key looks like AST1.<payload>.<signature>, pasted whole.': 'Dit is geen Asteria-licentie. Een sleutel ziet eruit als AST1.<inhoud>.<handtekening>, in zijn geheel geplakt.',
    'The key names a signing key this build does not know ({kid}). Update the extension, or the key is not for this product.': 'De sleutel verwijst naar een tekensleutel die deze versie niet kent ({kid}). Werk de extensie bij, of de sleutel is niet voor dit product.',
    'The signature does not verify. The key was altered in copying, or it was not issued for Asteria.': 'De handtekening klopt niet. De sleutel is bij het kopiëren veranderd, of hij is niet voor Asteria uitgegeven.',
    'Plus expired on {date}. Nothing was removed; the Plus screens explain what they would show.': 'Plus is verlopen op {date}. Er is niets verwijderd; de Plus-schermen leggen uit wat ze zouden tonen.',
    'Plus, valid until {date} ({days} days).': 'Plus, geldig tot {date} ({days} dagen).',
    'Renew before then to keep the weekly bundle coming.': 'Verleng voor die datum om de wekelijkse bundel te blijven ontvangen.',
    'Last value': 'Laatste waarde',
    'DEGIRO says': 'DEGIRO zegt',
    'DEGIRO (derived)': 'DEGIRO (afgeleid)',
    Difference: 'Verschil',
    'What is free, and what Plus adds': 'Wat gratis is, en wat Plus toevoegt',
    'Everything computed from your own DEGIRO data is free and stays free. Plus adds external data and convenience, and never changes a computed figure: a paying and a free reader see the same portfolio value to the cent.':
      'Alles wat uit je eigen DEGIRO-data wordt berekend is gratis en blijft gratis. Plus voegt externe data en gemak toe en verandert nooit een berekend getal: een betalende en een gratis gebruiker zien dezelfde portefeuillewaarde tot op de cent.',
    'In preparation': 'In voorbereiding',
    'Planned at € 29 a year, paid once a year through a payment partner. No account, no login, no telemetry: a key by mail, checked offline in the extension.':
      'Gepland op € 29 per jaar, eens per jaar afgerekend via een betaalpartner. Geen account, geen login, geen telemetrie: een sleutel per mail, offline gecontroleerd in de extensie.',
    Part: 'Onderdeel',
    Free: 'Gratis',
    you: 'jij',
    'Why here': 'Waarom hier',
    yes: 'ja',
    no: 'nee',
    identical: 'identiek',
    'not fetched': 'niet opgehaald',
    'weekly, signed': 'wekelijks, getekend',
    'Reconstruction, reconciliation, every chart, Holdings, Performance, Notices': 'Reconstructie, reconciliatie, alle grafieken, Posities, Rendement, Meldingen',
    'From your own DEGIRO data': 'Uit je eigen DEGIRO-data',
    'Dividend per share, raises and cuts, rhythm, expected annual income, track record': 'Dividend per aandeel, verhogingen en verlagingen, ritme, verwacht jaarinkomen, staat van dienst',
    'From your own payments': 'Uit je eigen uitkeringen',
    'Export, bug report, interop': 'Export, bugrapport, interop',
    'Your data, your file': 'Jouw data, jouw bestand',
    'Data bundle: dividend history, fundamentals, TER, benchmark ETFs': 'Databundel: dividendhistorie, fundamentals, TER, benchmark-ETF\u2019s',
    'External data, running cost': 'Externe data, doorlopende kosten',
    'Against the market (benchmark)': 'Tegenover de markt (benchmark)',
    'The first part with a server side': 'Het eerste onderdeel met een serverkant',
    'Safety per payment, income per bucket': 'Veiligheid per uitkering, inkomen per vak',
    'Payout and debt come from the bundle': 'Payout en schuld komen uit de bundel',
    'Reclaimable withholding tax per country': 'Terug te vorderen bronbelasting per land',
    'Country and treaty rate from the bundle': 'Vestigingsland en verdragstarief uit de bundel',
    'Backup and restore of the raw store': 'Back-up en herstel van je ruwe gegevens',
    'Convenience; touches no figure': 'Gemak; raakt geen getal',
    'Year review as PDF and shareable card': 'Jaaroverzicht als PDF en deelbare kaart',
    Convenience: 'Gemak',
    'Box 3 reference date and dividend tax': 'Box 3-peildatum en dividendbelasting',
    'Two figures you copy into the tax return': 'Twee cijfers die je overtypt in de aangifte',
    'Portfolio value, result, every computed figure': 'Portefeuillewaarde, resultaat, elk berekend getal',
    'Rule 6: the same cent for everyone': 'Regel 6: dezelfde cent voor iedereen',
    'What Plus will never do': 'Wat Plus nooit doet',
    'Send your portfolio, or any part of it, to a server': 'Je portefeuille, of een deel ervan, naar een server sturen',
    'Ask for an account, a password or a device registration': 'Een account, wachtwoord of apparaatregistratie vragen',
    'Fetch a selection of the bundle (that would reveal your positions); always the whole bundle': 'Een selectie van de bundel ophalen (dat verraadt je posities); altijd de hele bundel',
    'Telemetry or usage measurement, anonymised or not': 'Telemetrie of gebruiksmeting, ook niet geanonimiseerd',
    'Compute a free figure differently from a paid one': 'Een gratis getal anders berekenen dan een betaald getal',
    'Log in to DEGIRO; the extension reads a session your browser already holds, and that stays so': 'Inloggen bij DEGIRO; de extensie leest een sessie die je browser al heeft, en dat blijft zo',
    Syncing: 'Bezig',
    ...
    'Top 3 payers': 'Top 3 betalers',
    'no regular dividend in the last 12 months': 'geen regulier dividend in de laatste 12 maanden',
    'all {n} positions': 'alle {n} posities',
    'of the last 12 months’ regular gross dividend · {names}': 'van het reguliere brutodividend van de laatste 12 maanden · {names}',
    '{n} payments not attributable': '{n} uitkeringen niet toe te rekenen',
    'recovered {date}, {days} days under water': 'hersteld {date}, {days} dagen onder water',
    'not yet recovered, {days} days and counting': 'nog niet hersteld, {days} dagen en tellend',
    'The share of the last twelve months’ regular gross dividend, in EUR, that came from the three largest payers. Special dividends are left out. A high share means one cut is a large part of the income; the per-position table below is where to see which one.':
      'Het aandeel van het reguliere brutodividend van de laatste twaalf maanden, in EUR, dat van de drie grootste betalers kwam. Bijzondere uitkeringen tellen niet mee. Een hoog aandeel betekent dat één verlaging een groot deel van het inkomen raakt; in de tabel per positie hieronder zie je welke.',
