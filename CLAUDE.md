...
```
manifest.json           MV3
src/sw.js               service worker: alarms, message router (checks the sender's
                        origin before answering). Owns no state.
src/content/boot.js     US-91: classic shim on trader.degiro.nl that imports the two
                        modules below; MV3 content scripts cannot be modules.
src/content/banner.js   the strip and the toast on trader.degiro.nl. Reads nothing from
                        the page; its text is decided in src/lib/bannermodel.js.
src/content/readywatch.js  US-113: waits for the DEGIRO tab to go quiet (resource-timing
                        counts, never a URL), then asks the worker to sync.
src/content/site.js     US-97: on asteria.prulwerk.nl, announces the installed version
                        and relays one request — open the demo. Nothing else.
...
src/lib/combine.js      per-broker engine results -> one. Pure; pnl adds, identically.
src/lib/withholding.js  US-106: effective dividend withholding rate per position. Pure;
                        an unrecognised input is null and says so.
src/lib/anon.js         US-46: what a hidden amount looks like — replacement, not blur.
src/lib/snapshot.js     US-47: what may be on a shareable card. Pure; the key set is
                        the leak test.
src/lib/describe.js     US-71: what a chart says to a screen reader — three shapes.
src/lib/report.js       the bug report: an allowlist per warning code, counts not names.
src/lib/bannermodel.js  US-91: the strip's line and tone from four status fields. Pure.
src/lib/readiness.js    US-113: is the tab quiet enough to sync — timestamps in. Pure.
src/lib/brokers/        the broker boundary: index.js is the registry, degiro.js a
                        façade over session/degiro/parse. Nothing above it names a broker.
...
src/lib/diagnose.js     step-by-step connection check. Output must stay free of
                        session ids, account numbers, amounts and cash-row wording — it
                        is meant to be pasted into a bug report.
...
tools/                  fixture generator, HAR converter, dev server, icon generator,
                        and the four checks npm test runs first (leaks, palette, type,
                        backlog)
```

**The whole repository is published.** GitHub Pages serves the root of `main` at
`demo.asteria.prulwerk.nl` (see `CNAME`) so the demo can run from `index.html` — which means
`docs/`, `fixtures/` and `test/` are on the public web too, not just the app. Nothing that is not
synthetic may ever be committed, and `tools/check-leaks.mjs` runs before the tests for that reason.
