...
**Owner decision, 2026-08-23 — one extension, not a second product.** The idea of "Asteria
Dividend Tracker" as a separate, independently-hosted app (reached by a button from Income &
cost, handed the extension's data over a relay) was considered and dropped in favour of the
simpler shape: **a new tab in this same extension**, the same pattern US-99 and US-106 already
used for Rendement and Income & cost. A separate hosted page would need a `postMessage` relay,
would lose its data the moment the tab closes, and would need its own repo, hostname, DNS record
and certificate — all for no benefit, since nothing about this feature needs to be reachable
outside the extension. US-108 and US-109's UI lands on a new **Dividends** tab of its own, not
folded into Income & cost, which already carries the deposits chart, the dividend-per-month chart
and US-106's withholding table.

The same reasoning trims US-104: no new repo and no new subdomain either. `Teamkiezeer` is this
project's own precedent for exactly this — a `pipeline/`, `data/` and `.github/workflows/` set
living inside an existing site's repo, unrelated to the page it serves. The bundle can live in
whichever repo ends up hosting the pipeline (this one, or `asteria.prulwerk.nl`, if that repo is
ever attached to a session) and be reachable from its existing hostname, or even straight from
`raw.githubusercontent.com` — no Cloudflare record, no GitHub Pages certificate wait, nothing to
provision before the pipeline itself exists.
...
- **Layer B — a static, read-only bundle, hosted from whichever repo ends up carrying the
  pipeline (see the 2026-08-23 decision above — no new repo or subdomain needed for this).**
  Per-ISIN reference data
...
### US-104 — The bundle pipeline (E9-2) *(new, refined — three free sources feeding one bundle, no new repo or subdomain)*

As a builder, I want a scheduled GitHub Action that assembles per-ISIN reference data from GLEIF,
SEC EDGAR and a manually-curated table, and publishes it as JSON reachable over a plain fetch, so
the extension gets fundamentals without a backend and without a vendor contract — the same
`pipeline/` + `data/` + weekly-workflow shape already running for `Teamkiezeer`, living inside an
existing repo rather than a new one (2026-08-23 decision, above).
...
Lands on the new **Dividends** tab (2026-08-23 decision, above) — not Income & cost, which is
already carrying the deposits chart, the dividend-per-month chart and US-106's withholding table.
