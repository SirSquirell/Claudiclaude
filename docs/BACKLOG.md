...
### US-26 — Calls, bought puts and crypto: what is actually tested *(new)*

**As someone whose account is not the one this was built against, I want to know which instrument
types have been verified and which have only ever been assumed.**

#### The honest table

Asked whether calls, bought puts and crypto are untested. Partly — and the two halves of "tested"
have to be separated, because one of them proves much less than it sounds like.

| | Synthetic (`npm run audit:synthetic`) | A real account |
|---|---|---|
| **Written (short) puts** | ✓ | ✓ — the options account this was all verified against holds 27, and every contract in it is a `P` |
| **Long calls** | ✓ generated by `make-account.mjs` | ✗ **never** |
| **Short calls** | ✓ | ✗ **never** |
| **Bought (long) puts** | ✓ | ✗ **never** |
| **Crypto, in any form** | ✗ | ✗ — the word does not appear anywhere in this repo |
...
- ☐ `docs/LIMITATIONS.md` says which instrument types have real-account evidence and which have
  only synthetic evidence. It currently implies more than is true by not distinguishing them.
- ☐ An export from an account that has held **calls** is run through `npm run audit`, and the
  finding is written down either way. This is one afternoon and it needs a volunteer, not a
  sprint.
- ☐ The same for a **bought put**, which is a different sign from the 27 written ones.
- ☐ Before any crypto work: settle which broker it is even about.
- ☐ If crypto proceeds: a fractional-quantity case in the synthetic account, and
  `deriveContractSizes` reports `unresolved` rather than a wrong integer when the settled amount
  is too small to measure through — the same rule `MIN_FX_LEG` now applies to rates.

#### What this story is not

A promise that calls are broken. The arithmetic says they work and the synthetic run agrees. It is
a statement that **nobody has ever pointed this at a real one**, and that the README and
`LIMITATIONS.md` should say so in those words rather than leaving a reader to assume the coverage
is uniform.
