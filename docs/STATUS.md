...
| US-37 | **Trading 212 R1 — page context PASSED and the logged-out control PASSED, 2026-08-12.** `in/include=200, in/omit=401, out/include=401`: the session is cookie-borne | The service-worker half. The probe is built and lives on `claude/degiro-portfolio-spike-7x5d4h` only — the host permission it needs does not go to testers while the question is open |
