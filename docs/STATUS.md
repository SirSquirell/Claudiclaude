...
| US-66 | **Disconnect and freeze** — forget `userToken`, `intAccount`, `displayName` and disarm the periodic sync, while every figure stays on screen labelled with its as-of date. Explicitly *not* our wipe, which resyncs; the cookie was never ours and stays untouched | Nothing. Owner request, clarified in chat: *"ja wipe, maar dat de cijfers freezen"*. Carries an `i` on the button — three sentences on the session, what disconnect forgets, what it leaves alone |
